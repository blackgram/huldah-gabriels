  ...
  return (
    <div className='fixed w-full top-0 left-0 px-4 flex items-center justify-between text-white font-urbanist bg-white z-50'>
        ...
        <div className='flex justify-evenly bg-primary rounded-b-2xl text-[8px] px-4 border-b-primary border-b border-t border-t-primary'>
            <div className={`p-2 cursor-pointer ${activeMenu === 'home' && 'font-bold'}`} onClick={onScrollToHome}>Home</div>
            <div className={`p-2 cursor-pointer ${activeMenu === 'products' && 'font-bold'}`} onClick={onScrollToProducts}>Products</div>
            <div className={`p-2 cursor-pointer ${activeMenu === 'review' && 'font-bold'}`} onClick={onScrollToReview}>Review</div>
            <div className={`p-2 cursor-pointer ${activeMenu === 'about' && 'font-bold'}`} onClick={onScrollToAbout}>About Us</div>
        </div>
