...
export default async function handler(
  ...
  try {
    ...
    if (!orderDetails || !orderDetails.customerEmail) {
      return res.status(400).json({ error: 'Order details and customer email are required' });
    }
    ...
    const emailHtml = `
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #946A2E 0%, #B8860B 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          ...
          <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              ...
              <p style="margin: 5px 0;"><strong>Order ID:</strong> ${sessionId.substring(0, 20)}...</p>
              <p style="margin: 5px 0;"><strong>Order Date:</strong> ${new Date().toLocaleDateString()}</p>
              <p style="margin: 5px 0;"><strong>Status:</strong> <span style="color: #22c55e; font-weight: bold;">Confirmed</span></p>
            </div>
            ...
            <div style="background: #e8f5e9; padding: 20px; border-radius: 8px; border-left: 4px solid #22c55e; margin-bottom: 20px;">
              <h3 style="color: #22c55e; margin-top: 0;">📦 What's Next?</h3>
              <p style="margin: 5px 0;">Your order is being prepared and will be shipped within 1-2 business days.</p>
              <p style="margin: 5px 0;">You'll receive a tracking number via email once your order ships.</p>
    ...
    const info = await transporter.sendMail({
      ...
      subject: `Order Confirmation - Order #${sessionId.substring(0, 8)}`,
      html: emailHtml,
    });
